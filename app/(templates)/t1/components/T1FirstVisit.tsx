    ...
    <ol className="relative" aria-label="How a first visit works">
      {STEPS.map((step, i) => {
        ...
        return (
          <li
            key={step.title}
            className="relative border-l border-[var(--t1-hairline)] pb-10 pl-6 last:pb-2 md:pl-10"
          >
            <span
              aria-hidden="true"
              className={`absolute -left-[5px] top-1 h-[9px] w-[9px] bg-[var(--t1-red)] transition-transform duration-300 ${
                open ? "scale-150" : ""
              }`}
            />
            <Fade delay={i * 0.06}>
              <div className={`max-w-xl ${i % 2 === 1 ? "md:ml-[14%]" : ""}`}>
                ...
                >
                  ...
                  <span className="mt-2 flex items-baseline justify-between gap-4">
                    <span className="font-t1-press text-2xl uppercase leading-none text-[var(--t1-ink)] transition-colors duration-300 group-hover:text-[var(--t1-red)] md:text-3xl">
                      {step.title}
                    </span>
                    <span
                      aria-hidden="true"
                ...
                <AnimatePresence initial={false}>
                  {open && (
                    ...
                    >
                      <p className="mt-2 font-sans text-sm leading-relaxed text-[var(--t1-stone)] md:text-base">
                        {step.note}
                      </p>
                    </motion.div>
                  )}
