  ...
  return (
    <footer className="py-12 px-8 bg-brand-mainText">
      <div className="max-w-6xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          <div className="md:col-span-2">
            ...
            <div className="flex gap-3 mt-4">
              {footerLinks.map((link) => (
                <a key={link.label} href={link.href} className="text-brand-canvas/50 hover:text-brand-accent text-sm transition-colors">
                  {link.label}
                </a>
              ))}
            </div>
          ...
          <div>
            ...
            <a
              ...
              className="inline-block mt-4 px-5 py-2.5 rounded-lg bg-brand-primary text-brand-canvas font-semibold text-sm hover:brightness-110 transition-all"
            >
              Book Online
            </a>
          </div>
        ...
        {/* Bottom Bar */}
        <div className="mt-8 pt-8 border-t border-brand-canvas/10 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-brand-canvas/40 text-sm">
            © {new Date().getFullYear()} {location.practiceNameGBP}. All rights reserved.
          </p>
          <div className="flex gap-6 text-brand-canvas/40 text-sm">
            <a href="#" className="hover:text-brand-canvas transition-colors">Privacy</a>
